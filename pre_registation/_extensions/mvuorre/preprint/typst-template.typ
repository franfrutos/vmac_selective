#let preprint(
    ...
    header: locate(
        ...
        loc => if [#loc.page()] == [1] {
          ...
          box(
            ...
            [
              #date
              #if citation != none {linebreak(); link("https://" + citation.doi, "https://" + citation.doi)}
            ]
          )
          box(
            inset: 0.2em,
            if branding == "psyarxiv" {
              image("psyarxiv.svg", width: 2.25em) // small change in the branding
            } else {
              image("osf.svg", width: 2.25em)
            }
          )
        } else {
  ...
  let titleblock(
    ...
    #align(center)[
      #block(width: width, above: above, below: below)[
        #text(weight: weight, size: size, style: style)[#body]
      ]
    ]
  ]

  if title != none {
    titleblock(title, size: 1.35em, below: 0.1em)
  }
  
  if subtitle != none {
    titleblock(subtitle, size: 1.25em, weight: "regular", below: 2em, style: "italic")
  }
  ...
  if affiliations != none {
    titleblock(
      weight: "regular", size: 1em, below: 2em,
